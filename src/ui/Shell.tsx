...
// ─── Dusk Skyline SVG ─────────────────────────────────
export function DuskSkyline({ height = 180, opacity = 1 }: { height?: number; opacity?: number }) {
  return (
...
// ─── Stars ────────────────────────────────────────────
export function Stars({ count = 30, topBias = 0.6 }: { count?: number; topBias?: number }) {
  const stars = useMemo(() =>
...
// ─── Orb ──────────────────────────────────────────────
export function Orb({ size = 80, style = {} }: { size?: number; style?: React.CSSProperties }) {
  return (
...
// ─── Icon ─────────────────────────────────────────────
type IconName =
  | 'home' | 'map' | 'users' | 'stars' | 'flame' | 'chev-right' | 'chev-left' | 'chev-down'
  | 'play' | 'pause' | 'target' | 'spark' | 'check' | 'lock' | 'bolt' | 'hint' | 'crown'
  | 'book' | 'angle' | 'tower' | 'heart' | 'close' | 'send' | 'globe'
...
export function Icon({ name, size = 20, color = 'currentColor' }: { name: IconName; size?: number; color?: string }) {
  ...
  switch (name) {
    ...
    case 'pause':      return <svg {...common}><rect x="6" y="4" width="4" height="16" rx="1" fill={color}/><rect x="14" y="4" width="4" height="16" rx="1" fill={color}/></svg>
    ...
    case 'bolt':       return <svg {...common}><path d="M13 2L4 14h7l-1 8 9-12h-7l1-8z" fill={color}/></svg>
...
// ─── Tab Bar ──────────────────────────────────────────
type TabId = 'home' | 'world' | 'raid' | 'progress'

export function TabBar({ active, onNav }: { active: TabId; onNav: (id: TabId) => void }) {
